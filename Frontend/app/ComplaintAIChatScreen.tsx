...
export default function ComplaintAIChatScreen() {
  ...
  const loadHistory = useCallback(async () => {
    try {
      const response = await nlpService.getBotComplaintsHistory(communityId);
      if (response?.success) {
  ...
  const handleSend = async (text: string) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          id: `err-${Date.now()}`,
          text: 'Sorry, I encountered an error processing your complaint query.',
          isBot: true,
          timestamp: new Date(),
        },
      ]);
  ...
  return (
    <AIChatUI
      ...
      emptyStateIcon="⚠️"
      emptyStateText={`How can I help you with complaints in ${communityName}? I can help you draft complaints, check status, or analyze trends.`}
      accentColor="#f59e0b"
    />
  );
}
