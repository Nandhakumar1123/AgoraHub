...
export default function ComplaintAIChatScreen() {
  ...
  const loadHistory = useCallback(async () => {
    try {
      const response = await nlpService.getBotHistory(communityId, 100, 'complaints');
      if (response?.success) {
      ...
      console.error('Failed to load complaint AI history:', error);
    }
  }, [communityId]);
    ...
    async (dateKey: string) => {
      ...
      const hydrated = await Promise.all(
        raw.map(async (m) => {
          ...
          const cached = sanitizeTamilSummary(await loadTamilSummaryCache('complaints', communityId, m.historyId));
  ...
  const handleSend = async (text: string) => {
    ...
    try {
      const response = await nlpService.askBotComplaints(text, communityId);
      if (response?.success) {
      ...
      console.error('Complaint AI query failed:', error);
      setMessages((prev) => [
        ...prev,
        {
          id: `err-${Date.now()}`,
  ...
  const handleClear = async () => {
    try {
      await nlpService.clearBotHistory(communityId, 'complaints');
      setMessages([]);
      setHistoryItems([]);
      setSelectedThreadId(null);
      setIsNewChat(true);
    } catch (error) {
      console.error('Failed to clear complaint chat history:', error);
    }
  };
  ...
  const handleEdit = async (id: string, newText: string) => {
    ...
    try {
      const response = await nlpService.updateBotHistoryItem(communityId, msg.historyId, newText, 'complaints');
      if (response?.success) {
        setMessages((prev) =>
      ...
      console.error('Failed to edit complaint message:', error);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await nlpService.deleteBotHistoryItem(communityId, msg.historyId, 'complaints');
      if (response?.success) {
        setMessages((prev) => prev.filter((m) => m.id !== id));
      ...
      console.error('Failed to delete complaint message:', error);
  ...
  const runTranslate = async (id: string, targetLanguage: 'Tamil' | 'Tanglish') => {
    ...
    try {
      ...
      if (response?.success) {
        ...
        if (msg.historyId) {
          await saveTamilSummaryCache('complaints', communityId, msg.historyId, String(translated).trim());
        }
      } else {
  ...
  const handleTranslate = async (id: string) => {
    ...
    if (prefetch) {
      ...
      await saveTamilSummaryCache('complaints', communityId, msg.historyId, prefetch);
      return;
    }
  ...
  return (
    <AIChatUI
      ...
      emptyStateIcon="⚠️"
      emptyStateText={isNewChat ? `How can I help you with complaints in ${communityName}? I can help you draft complaints, check status, or analyze trends.` : "Select a day from the list to view history."}
      accentColor="#f59e0b"
    />
  );
}
