...
export default function PetitionDetailsScreen() {
  ...
  const fetchFollowUpMessages = async () => {
    try {
      ...
      if (!token) return;
      const response = await fetch(`${BASE_URL}/follow-up-messages?petitionId=${petitionId}`, {
  ...
  const handleSelectStatus = async (newStatus: PetitionStatus) => {
    ...
    try {
      ...
      Alert.alert(
        'Success', 
        `Petition status updated to ${newStatus.toLowerCase()} successfully!`,
        [
          {
    ...
    <View style={[styles.root, themeStyles.root]}>
      <ScrollView style={[styles.container, themeStyles.root]}>
        ...
        <View style={styles.content}>
          ...
          {(userRole === 'HEAD' || userRole === 'ADMIN' || currentUserId === petition.author_id) && (
              ...
              {followUpMessages.length === 0 ? (
                ...
                <View style={styles.followUpTimeline}>
                  {followUpMessages.map((msg, index) => {
                    const isSenderAdmin = msg.sender_id !== petition.author_id;
                    return (
                      <View 
