...
export default function PetitionAIChatScreen() {
  ...
  const loadHistory = useCallback(async () => {
    try {
      const response = await nlpService.getBotHistory(communityId, 100, 'petitions');
      if (response?.success) {
    ...
    async (dateKey: string) => {
      ...
      const hydrated = await Promise.all(
        raw.map(async (m) => {
          ...
          const cached = sanitizeTamilSummary(await loadTamilSummaryCache('petitions', communityId, m.historyId));
  ...
  const handleClear = async () => {
    try {
      ...
      console.error('Failed to clear petition chat history:', error);
    }
  };
  ...
  const runTranslate = async (id: string, targetLanguage: 'Tamil' | 'Tanglish') => {
    ...
    try {
      ...
      if (response?.success) {
        ...
        if (msg.historyId) {
          await saveTamilSummaryCache('petitions', communityId, msg.historyId, String(translated).trim());
        }
      } else {
  ...
  const handleTranslate = async (id: string) => {
    ...
    if (prefetch) {
      ...
      await saveTamilSummaryCache('petitions', communityId, msg.historyId, prefetch);
      return;
    }
  ...
  return (
    <AIChatUI
      ...
      emptyStateIcon="📋"
      emptyStateText={isNewChat ? `How can I help you with petitions in ${communityName}? I can explain rules, analyze support, or suggest next steps.` : "Select a day from the list to view history."}
      accentColor="#10b981"
    />
  );
}
